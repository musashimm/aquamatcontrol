...
QByteArray ControlComponent::getAllAsArray() {
    QByteArray a;
    for (int i = 0; i < controls.size(); ++i) {
		a.append(controls.at(i)->getAsArray());
	}
	return a;
}

void ControlComponent::saveAll(QSettings* s) {
    for (int i = 0; i < controls.size(); ++i) {
		controls.at(i)->save(s);
	}
}

void ControlComponent::loadAll(QSettings* s) {
     for (int i = 0; i < controls.size(); ++i) {
		controls.at(i)->load(s);
	}
}

void ControlComponent::changeOutputNameForAll(int id,QString name) {
    for (int i = 0; i < controls.size(); ++i) {
		controls.at(i)->changeOutputName(id,name);
	}
}
